...
class World(object):
    def __init__(self, size, custom, start_position, start_direction_index, food_position):
        ...
        self.world[0, range(0, self.size[0])] = self.WALL
        self.world[range(0, self.size[0]), 0] = self.WALL
        self.world[self.size[0] - 1, range(0, self.size[0])] = self.WALL
        self.world[range(0, self.size[0]), self.size[0] - 1] = self.WALL
    ...
    def init_snake(self):
        ...
        if not self.custom:
            ...
            start_position = (random.randrange(4, self.size[0] - 3), (random.randrange(4, self.size[0] - 3)))
            # Choose a random direction index
            start_direction_index = random.randrange(0, 4)
    ...
    def init_food(self):
        ...
        self.current_available_food_positions = self.available_food_positions.difference(self.snake.blocks)
        if not self.custom:
    ...
    def move_snake(self, action):
        ...
        if self.snake.alive:
            ...
            for item in range(len(self.snake.blocks)):
                if item == 0:
                    continue
                else:
                    if new_snake_head == self.snake.blocks[item]:
                        self.snake.alive = False
            #  Check if snake eats the food
            if new_snake_head == self.food_position:
                # Remove old food
                self.world[new_snake_head] = 0  # асайнемо координату світу до 0, так як голова в координаті їжі
                # Add tail again
                self.snake.blocks += [old_snake_tail]
                # self.snake.blocks += [old_snake_tail]  # аппендимо координату старого хвоста
